...
export function matchGlob(glob, file) {
    if (glob.endsWith("/**"))
        return file === glob.slice(0, -3) || file.startsWith(glob.slice(0, -2));
    if (glob.includes("*")) {
...
export function candidatesFor(docsDir, changed) {
    let files = [];
    try {
        files = readdirSync(docsDir, { recursive: true }).filter((f) => String(f).endsWith(".md"));
    }
    catch {
        return [];
    }
