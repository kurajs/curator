...
const sh = (cmd, args, cwd = process.cwd(), env) => execFileSync(cmd, args, { cwd, encoding: "utf8", env: env ?? process.env }).trim();
...
const inp = (key, action) => ((process.env[`CURATOR_${key}`] ?? "").trim() || core.getInput(action));
...
async function run() {
    ...
    const committer = {
        name: inp("COMMITTER_NAME", "committer-name") || "Mori",
        email: inp("COMMITTER_EMAIL", "committer-email") || "mori@kura.build",
    };
    if (apiKey)
...
function subjectFor(custom, editedPaths, docsDir, changed) {
    if (custom)
        return custom;
    const names = [
        ...new Set(editedPaths.map((p) => (p.startsWith(`${docsDir}/`) ? p.slice(docsDir.length + 1) : p).replace(/\.mdx?$/i, "").split("/").pop())),
    ];
...
async function sameRepo({ cwd, docsDir, base, mode, changed, backendName, resolveOpts, token, committer, commit }) {
    ...
    try {
        sh("git", [...gitId(committer), "cherry-pick", docsCommit]);
    }
    catch {
    ...
    const url = sh("gh", ["pr", "create", "--title", subject, "--body", prBody(changed, candidates, docsDir, summary), "--head", branch, "--base", base], cwd, { ...process.env, GH_TOKEN: token });
...
async function crossRepo({ cwd, docsDir, docsRepo, docsRef, changed, backendName, resolveOpts, token, committer, commit }) {
    ...
    const url = sh("gh", ["pr", "create", "--repo", docsRepo, "--title", subject, "--body", prBody(changed, candidates, docsDir, summary, process.env.GITHUB_REPOSITORY ?? ""), "--head", branch, "--base", docsRef], checkout, { ...process.env, GH_TOKEN: token });
    core.info(`Opened docs PR in ${docsRepo}: ${url}`);
    core.setOutput("pr-url", url);
}
function editPrompt(changed, pages, scope) {
    return (`${moriPrompt("maintainer")}\n\n` +
...
function prBody(changed, candidates, docsDir, summary, fromRepo = "") {
