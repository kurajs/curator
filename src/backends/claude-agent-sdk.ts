...
function inScope(cwd: string, filePath: string | undefined, allow: string[]): boolean {
...
export function claudeAgentSdk(model?: string): AgentBackend {
  return {
    name: "claude-agent-sdk",
    async run({ cwd, prompt, allowEdits }) {
      const deny = async (input: any) => {
      ...
      for await (const m of query({
        ...
        if ("result" in (m as any)) summary = String((m as any).result);
      }
      return { summary };
    },
  };
